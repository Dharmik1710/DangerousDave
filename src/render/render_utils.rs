...
pub fn create_visible_tile_batches(state: &mut GameState, app_resources: &AppResources) {
    ...
    for row in 0..total_rows { // Always loop over 10 fixed rows
        for col in start_col..end_col.min(total_columns) {
            ...
            let dest_rect = Rect::new(dest_x, dest_y, scaled_tile_size as u32, scaled_tile_size as u32);

            // Insert into the HashMap
            batches.entry(tile_index)
